...
if __name__ == "__main__":
    ...
    # while True: 
        ...
        # if choice == "1":
            ...
            #         task = TaskFactory.create_task(task_title, task_description, due_date)
            #         task_list.add_task(task)
            ...
            #         task = TaskFactory.create_recurring_task(task_title, task_description, due_date, interval)
            #         task_list.add_task(task)
            #         print(f"'{task_title}' has been added to your to-do list.\n")
            #         break
            #     elif sub_choice == "3":
        ...
        # elif choice == "3":
            ...
            #         if index < 1 or index > len(task_list.tasks):
            #             raise IndexError("That task number doesn't exist.")

            #         # Perform removal
            #         removed_task_title = task_list.tasks[index - 1].title  # get title before removal
            #         print(f"Removed -----: {removed_task_title}")
            #         task_list.remove_task(index)
            #         # Remove from CSV too
            #         dao = TaskCsvDAO("tasks.csv")

            #         # Reload all tasks from CSV
            #         all_tasks = dao.get_all_tasks()

            #         # Keep only tasks that don’t match the removed one
            #         updated_tasks = [t for t in all_tasks if t.title != removed_task_title]

            #         # Save the new list back to CSV
            #         print("Your Updated Tasks:\n")
            #         for i, t in enumerate(updated_tasks):
            #             print(f"Task {i + 1}: {t}")
            #         dao.save_all_tasks(updated_tasks)
